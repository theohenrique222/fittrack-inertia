...
const showCircunferences = ref(true);

const form = useForm({
    recorded_at: props.measurement.recorded_at?.split(' ')[0] ?? '',
    weight: String(props.measurement.weight),
    height: String(props.measurement.height),
    neck: props.measurement.neck?.toString() ?? '',
    waist: props.measurement.waist?.toString() ?? '',
    hip: props.measurement.hip?.toString() ?? '',
    chest: props.measurement.chest?.toString() ?? '',
    thigh: props.measurement.thigh?.toString() ?? '',
    arm: props.measurement.arm?.toString() ?? '',
    forearm: props.measurement.forearm?.toString() ?? '',
    calf: props.measurement.calf?.toString() ?? '',
    shoulders: props.measurement.shoulders?.toString() ?? '',
    activity_level: props.measurement.activity_level,
    goal: props.measurement.goal,
    notes: props.measurement.notes ?? '',
});

function handleSubmit() {
    form.put(`/students/${props.student.id}/measurements/${props.measurement.id}`, {
        preserveScroll: true,
        onSuccess: () => {
            emit('close');
        },
        ...
        <div class="sticky bottom-0 border-t border-neutral-200 bg-white px-6 py-4 dark:border-neutral-700 dark:bg-neutral-900">
            <div class="flex gap-3">
                ...
                <Button
                    ...
                    :disabled="form.processing"
                >
                    <template v-if="form.processing">Salvando...</template>
